# Guia de Construccion y Publicacion: API Backend (Node.js)

Este directorio contiene el codigo fuente de la API Backend desarrollada en Node.js para la Tienda de Perritos. El backend se encarga de procesar la logica de negocio, interactuar con la base de datos MySQL y servir datos al frontend.
...
3.  **Repositorio ECR:** Haber ejecutado la fase de Terraform en la carpeta `eks/` para aprovisionar el repositorio del backend.
...
Ejecuta los siguientes comandos desde este directorio (`apps/backend`):

1.  **Construir la imagen localmente** (forzando arquitectura compatible):
    ```bash
    docker buildx build --platform linux/amd64 -t intro-devops-lab-backend .
    ```

2.  **Etiquetar la imagen** apuntando a tu repositorio de ECR (reemplaza `<ACCOUNT_ID>`):
    ```bash
    docker tag intro-devops-lab-backend <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/intro-devops-lab-backend:eks-v1
    ```

3.  **Subir la imagen a la nube:**
    ```bash
    docker push <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/intro-devops-lab-backend:eks-v1
    ```
...
1.  **Construir la imagen:**
    ```bash
    podman build --platform linux/amd64 -t intro-devops-lab-backend .
    ```

2.  **Etiquetar la imagen:**
    ```bash
    podman tag intro-devops-lab-backend <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/intro-devops-lab-backend:eks-v1
    ```

3.  **Subir la imagen:**
    ```bash
    podman push <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/intro-devops-lab-backend:eks-v1
    ```
