...
2. El workflow se dispara en `main`, `deploy` y `test`; antes de buildear las imágenes ejecuta los tests/coverage de Express y Flask (`npm test`, `npm run coverage`, `pytest --cov=app ...`). Además genera reportes HTML (`coverage/index.html` para Express y `htmlcov/index.html` plus `report.html` para Flask) y publica esos artefactos junto a los XML/JSON para que puedas descargarlos desde la ejecución fallida o exitosa del pipeline.
